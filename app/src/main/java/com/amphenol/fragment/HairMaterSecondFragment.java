                ...
                switch (v.getId()) {
                    case R.id.fragment_fast_requisition_main_inquire_bt:
                        ...
                        if (state) {//当前按钮状态为“清除”
                            ...
                            handleInquireMater(mPickItem.getBranch().getMater().getWarehouse(), mPickItem.getBranch().getMater().getUnit(), mPickItem.getBranch().getMater().getNumber(), mPickItem.getPick().getNumber(), mPickItem.getPickLine(), mSpinner.getSelectedItemPosition() == 0 ? "" : mStringArrayAdapter.getItem(mSpinner.getSelectedItemPosition()), mLocationEditText.getText().toString().trim(), mBranchEditText.getText().toString(), mPickItem.getQuantity(), mPickItem.getPick().getDepartment(), mPickItem.getPick().getWorkOrder(), mPickItem.getSequence());
                        }
                        break;
                    case R.id.fragment_fast_requisition_main_submit_bt:
                        if (mPickItem.getHairQuantity() == 0) {
                            ((BaseActivity) getActivity()).ShowToast("还未勾选中任何物料项");
                            return;
                        }
...
//                }
    ...
    private void handleInquireMater(String warehouse, String unit, String mate, String pickNumber, String pickLine, String shard, String location, String branch, double quantity, String department, String workOrder, String sequence) {
        if (!HairMaterSecondFragment.this.isVisible())
            return;
        Map<String, String> param = new HashMap<>();
        param.put("username", SessionManager.getUserName(getContext()));
