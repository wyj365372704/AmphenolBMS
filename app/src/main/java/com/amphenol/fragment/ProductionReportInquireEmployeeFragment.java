...
public class ProductionReportInquireEmployeeFragment extends Fragment {
    ...
    public static ProductionReportInquireEmployeeFragment newInstance(Job job,Employee employee, OptionCallBack optionCallBack) {
        ...
        mOnClickListener = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                switch (v.getId()) {
                    case R.id.bt:
                        if (employee.getState() == Employee.STATE_CODE_ON) {
                            handleRemoveEmployee(job.getWorkOrder().getNumber(),job.getStepNumber(),job.getProprNumber(),job.getJobNumber(),employee.getNumber());
                        }else{
                            handleAddEmployee(job.getWorkOrder().getNumber(),job.getStepNumber(),job.getProprNumber(),job.getJobNumber(),employee.getNumber());
                        }
                        break;
                }
            }
    ...
    private void handleAddEmployee(String work_order, String step_number, String propr_number, String job_number, String employee_number) {
        if (!this.isVisible())
            return;
        ...
        param.put("employee_number", employee_number);
        NetWorkAccessTools.getInstance(getContext()).getAsyn(CommonTools.getUrl(PropertiesUtil.ACTION_PRODUCTION_REPORT_EMPLOYEE_ADD, getContext()), param, REQUEST_CODE_OPTION, mRequestTaskListener);
    }

    private void handleRemoveEmployee(String work_order, String step_number, String propr_number, String job_number, String employee_number) {
        if (!this.isVisible())
            return;
        Map<String, String> param = new HashMap<>();
        param.put("username", SessionManager.getUserName(getContext()));
        ...
        param.put("job_number", job_number);
        param.put("employee_number", employee_number);
        NetWorkAccessTools.getInstance(getContext()).getAsyn(CommonTools.getUrl(PropertiesUtil.ACTION_PRODUCTION_REPORT_EMPLOYEE_REMOVE, getContext()), param, REQUEST_CODE_OPTION, mRequestTaskListener);
    }
