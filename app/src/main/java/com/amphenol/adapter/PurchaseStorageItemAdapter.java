 ...
 */
public class PurchaseStorageItemAdapter extends RecyclerView.Adapter {
    private List<Purchase.PurchaseItem.PurchaseItemBranchItem> date;
    private Context mContext;
    private onItemEventCallBack mOnItemEventCallBack;

    public PurchaseStorageItemAdapter(Context mContext, List<Purchase.PurchaseItem.PurchaseItemBranchItem> branches, onItemEventCallBack onItemEventCallBack) {
        this.date = branches;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        ...
        ((ViewHolderBody) holder).jhslTextView.setText(date.get(position).getBranch().getQuantity() + ""); //实收数量
        ((ViewHolderBody) holder).ssslEditText.setText(date.get(position).getBranch().getQuantity() + "");//入库数量
        if (position % 2 == 0)
